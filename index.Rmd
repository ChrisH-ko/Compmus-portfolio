...
ashkenazy_2 <- bind_rows(list(ashkenazy_4_1, ashkenazy_4_2)) %>%
...
```{r Chromagram_ashkenazy, fig.height=600}
ashkenazy_2_10 <- ggplot(bind_rows(chroma_ashkenazy),
                            aes(
      x = start + duration / 2,
      width = duration,
      y = pitch_class,
  ...
  geom_tile() +
  facet_wrap(~ sonata, dir = "v", scales = "free_x", ncol = 1, shrink = FALSE) +
  labs(x = "Time (s)", y = NULL, fill = "Magnitude", title = "Chromagram of various sonatas", subtitle = "Vladimir Ashkenazy") +
  theme_minimal() +
  scale_fill_viridis_c()
