...
alexeev_7 <- alexeev %>% filter(sonata == 7, movement == 1)

DTW_recordings <- lapply(list(ashkenazy_7, alexeev_7), chroma_preprocessing)

DTW_plot <- compmus_long_distance(
  DTW_recordings[[1]] |> mutate(pitches = map(pitches, compmus_normalise, "chebyshev")),
  DTW_recordings[[2]] |> mutate(pitches = map(pitches, compmus_normalise, "chebyshev")),
  feature = pitches,
  method = "euclidean"
) |>
  ggplot(
  ...
  geom_tile() +
  coord_equal() +
  labs(x = "Vladimir Ashkenazy", y = "Dmitri Alexeev", title = "Sonata 7") +
  theme_minimal() +
