...
<br>
The values for these parameters are `r final_rf$args$trees[[2]]`, `r final_rf$args$mtry[[2]]`, and `r final_rf$args$min_n[[2]]` respectively in our final model. At first glance, it seems that the training process went without a hitch. However, there clearly seem to be some wrong classifications in the testing set. It looks like the model has quite a bit of trouble classifying the middle sonatas. This does make sense, since there are only two pieces that belong to this class, and they are both very different from one another. 
