...
```{r tempogram_10}
# tempogram_10 <- plot_tempogram(alexeev_10_1)
```
...
### sonata 2_2
```{r load_alex_2}
alexeev_2_2 <- alexeev %>% filter(sonata == 3, movement == 2)
```

```{r tempogram_2, cache=TRUE}
tempogram_2 <- plot_tempogram(alexeev_2_2)
```

```{r display_plot}
tempogram_2 <- tempogram_2 +
  minute_scalex +
  labs(title = "Tempogram | sonata 3, mvt. 2", subtitle = "Dmitri Alexeev")

tempogram_3_2 <- tempogram_2
saveRDS(object = tempogram_3_2, file = "data/plots/tempogram_3_2.RDS")
```
```{r display_plot_3_2}
...
***
We can further analyse a recordings' tempo by...
