...
octatonic_keys <-
  tribble(~name, ~template,
          ...
          paste("octatonic 2", c_o_5[1]), circshift(octatonic_scale2, 6))
...
for (i in 2:12) {
  ...
  key_02 <-
  tribble(~name, ~template,
          paste("octatonic 2", c_o_5[i]), circshift(octatonic_scale2, j))
  
  octatonic_keys <- bind_rows(octatonic_keys, key_01, key_02)
...
key_2_1 <- plot_key_chords(zhukov_2_1, "normal_key") +
  labs(title = "Key estimation | sonata 2, mvt. 1", subtitle = "Igor Zhukov")
key_2_1
```
...
key_9_1 <- plot_key_chords(zhukov_9_1, "octatonic") +
  labs(title = "Key estimation | sonata 9", subtitle = "Igor Zhukov")
key_9_1
```
...
all_recordings %>%
  ...
  facet_wrap(~ sonata) + 
  labs(title = "Spotify key analysis", y = "", fill = "pianist", x = "key", alpha = "confidence")
```
