...
custom_theme <- theme(plot.background = element_rect(fill = "#353d3a"),
                      )
```
...
```{r functions_timbre, include=FALSE}
...
Structural details like this are hard to find in his other sonatas, as they are written . It is quite tough to define an overarching structure in his 8th sonata for instance. Instead, this sonata 
...
plot_grid(plotlist = list(selfsim_chroma_1, selfsim_timbre_1))
```
...
```{r}
selfsim_chroma_1 <- self_similarity(sonata_8, chroma=TRUE)
selfsim_timbre_1 <- self_similarity(sonata_8, chroma=FALSE)
