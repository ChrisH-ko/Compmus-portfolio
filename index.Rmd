...
new_pianists <- bind_rows(list(sofrontsky, hamelin)) %>%
    mutate(period = replace(sonata, sonata < 11, "Late")) %>%
    mutate(period = replace(period, sonata < 6, "Middle")) %>%
    mutate(period = replace(period, sonata < 4, "Early")) %>%
    mutate(period = factor(period, levels=c("Early", "Middle", "Late"))) %>%
...
features <-
  ...
  select(energy, valence, sonata:c12)
```

```{r train_test_split}
set.seed(2)
sonatas_split <- initial_split(features, prop = 2/3)
train_sonatas <- training(sonatas_split) %>% mutate(set = "Train set")
test_sonatas <- testing(sonatas_split) %>% mutate(set = "Test set")
```
...
To proceed, we need to divide our data in a set that will be used for training our model and one that will be used for testing its performance. Since these task rely on having access to sufficient and varied data, it might prove fruitful to extend our original corpus. The recordings of two new pianists have been added, namely Vladimir Sofronitsky and Marc-André Hamelin.
...
***
The actual model that we will use to predict a sonata's period will be a random forest. 
...
sonatas_juice <-
  recipe(
    identifier ~
      ...
      c01 + c02 + c03 + c04 + c05 + c06 +
      c07 + c08 + c09 + c10 + c11 + c12,
    data = sonatas_clustering
  ) |>
  step_center(all_predictors()) |>
  step_scale(all_predictors()) |> 
  # step_range(all_predictors()) |> 
  prep(sonatas_clustering |> mutate(identifier = str_trunc(identifier, 20))) |>
  juice() |>
  column_to_rownames("identifier")
```

```{r}
sonatas_dist <- dist(sonatas_juice, method = "euclidean")

data_for_sonata_clustering <- sonatas_dist |> 
  hclust(method = "average") |> # average for a balanced tree!
  dendro_data() 

playlist_data_for_join <- sonatas_clustering %>%
  select(identifier, period) %>%
  mutate(label = str_trunc(identifier, 20))

data_for_sonata_clustering$labels <- data_for_sonata_clustering$labels %>%
  left_join(playlist_data_for_join)

# Add factor so can use colouring! 
data_for_sonata_clustering$labels$label <- factor(data_for_sonata_clustering$labels$label)
```

```{r}
data_for_sonata_clustering |>
  ggdendrogram() +
  geom_text(data = label(data_for_sonata_clustering), aes(x, y, 
                                   label=label, 
                                   hjust=0, 
                                   colour=period), size=3) +
  coord_flip() + 
...
***
I'm probably going to remove this one since I don't think it's as interesting as the classification, so enjoy it while it lasts ¯\_(ツ)_/¯.
